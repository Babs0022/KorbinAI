...
Constraint: Avoid overly technical jargon.`;
    ...
    switch (currentSceneConfig.title) {
      case 'User Enters Goal':
        return (
          <div className="p-3 sm:p-4 md:p-6 space-y-2 sm:space-y-3 md:space-y-4 h-full flex flex-col justify-center">
            <div className="flex items-center text-sm sm:text-md md:text-lg font-semibold text-white">
              <Lightbulb className="mr-2 h-4 w-4 sm:h-5 md:h-6 sm:w-5 md:w-6 text-accent" />
              1. Define Your Goal
            </div>
            <Label htmlFor="goal-demo" className="text-xs sm:text-sm text-indigo-200">
            ...
            <Textarea
              ...
              readOnly
              rows={4}
              className="bg-slate-600/50 border-slate-500 focus-visible:ring-accent text-indigo-100 placeholder-indigo-300 text-xs sm:text-sm resize-none"
      ...
      case 'Answering Adaptive Survey':
        return (
          <div className="p-3 sm:p-4 md:p-6 space-y-2 sm:space-y-3 md:space-y-4 h-full flex flex-col justify-center">
            <div className="flex items-center text-sm sm:text-md md:text-lg font-semibold text-white">
              <Wand2 className="mr-2 h-4 w-4 sm:h-5 md:h-6 sm:w-5 md:w-6 text-accent" />
              2. Refine with Details
            </div>
            <div className="space-y-1.5 sm:space-y-2 md:space-y-3 text-xs sm:text-sm">
              <div>
                <Label htmlFor="q1-demo" className="text-indigo-200">Primary audience?</Label>
                <Input id="q1-demo" readOnly value={surveyAnswers.q1 as string || ''} className="mt-1 bg-slate-600/50 border-slate-500 text-indigo-100 placeholder-indigo-300 h-7 sm:h-8 text-xs sm:text-sm" placeholder="e.g., Developers, CEOs..." />
              </div>
              <div>
                <Label className="text-indigo-200">Desired tone?</Label>
                <RadioGroup value={surveyAnswers.q2 as string || ''} className="mt-1 flex flex-wrap gap-x-2 md:gap-x-3 gap-y-1">
                  {['Formal', 'Casual', 'Professional'].map(opt => (
                    <div key={opt} className="flex items-center space-x-1">
                      <RadioGroupItem value={opt} id={`q2-demo-${opt}`} checked={(surveyAnswers.q2  as string) === opt} readOnly className="border-accent data-[state=checked]:border-accent data-[state=checked]:text-accent h-3 w-3 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4"/>
                      <Label htmlFor={`q2-demo-${opt}`} className="font-normal text-indigo-200 text-[10px] sm:text-xs md:text-sm">{opt}</Label>
                    </div>
                  ))}
                </RadioGroup>
              </div>
               <div>
                <Label htmlFor="q3-demo-include" className="text-indigo-200">Keywords to include?</Label>
                <Input id="q3-demo-include" readOnly value={surveyAnswers.q3_include as string || ''} className="mt-1 bg-slate-600/50 border-slate-500 text-indigo-100 placeholder-indigo-300 h-7 sm:h-8 text-xs sm:text-sm" placeholder="e.g., innovation, efficiency" />
              </div>
              <div>
                <Label className="text-indigo-200">Constraints?</Label>
                <div className="mt-1 flex items-center space-x-2">
                    <Checkbox id="q5-demo-jargon" checked={(surveyAnswers.q5 as string[])?.includes('Avoid jargon')} readOnly className="border-accent data-[state=checked]:bg-accent data-[state=checked]:text-white h-3 w-3 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4" />
                    <Label htmlFor="q5-demo-jargon" className="font-normal text-indigo-200 text-[10px] sm:text-xs md:text-sm">Avoid jargon</Label>
                </div>
              </div>
            </div>
          </div>
      ...
      case 'AI Optimizing...':
        return (
            <div className="p-3 sm:p-4 md:p-6 flex flex-col items-center justify-center h-full space-y-2 sm:space-y-3">
                <Loader2 className="h-8 w-8 sm:h-10 md:h-12 sm:w-10 md:w-12 text-accent animate-spin" />
                <p className="text-sm sm:text-md md:text-lg text-indigo-200">Optimizing your prompt...</p>
                <div className="w-full max-w-[200px] sm:max-w-xs h-1.5 sm:h-2 bg-slate-600 rounded-full overflow-hidden">
                    <div className="h-full bg-accent transition-all duration-150" style={{width: `${optimizingProgress}%` }}></div>
                </div>
            </div>
        );
        ...
        return (
          <div className="p-3 sm:p-4 md:p-6 space-y-2 sm:space-y-3 md:space-y-4 h-full flex flex-col justify-center">
            ...
            <Textarea
              ...
              readOnly
              rows={4}
              className="bg-slate-600/50 border-slate-500 focus-visible:ring-accent text-indigo-100 placeholder-indigo-300 font-code text-[9px] sm:text-[10px] md:text-xs leading-snug resize-none"
              placeholder="Generating prompt..."
            />
             <div className="flex justify-end">
                <Button size="sm" variant="outline" className="border-accent text-accent hover:bg-accent/20 hover:text-accent h-7 sm:h-8 text-[10px] sm:text-xs md:text-sm">
  ...
  return (
    <section id="demo-video" className="py-12 md:py-16 lg:py-24 bg-slate-800">
      <Container className="text-center">
        <h2 className="font-headline text-3xl font-bold tracking-tight text-white sm:text-4xl">
          See BrieflyAI in Action
        </h2>
        <p className="mx-auto mt-3 sm:mt-4 max-w-xl sm:max-w-2xl text-md sm:text-lg text-indigo-200">
          Watch this interactive demo of how BrieflyAI optimizes your prompts.
        </p>
        <GlassCard
          className={cn(
            "mt-8 sm:mt-10 aspect-video overflow-hidden shadow-2xl bg-slate-700/70 backdrop-blur-lg border-slate-600",
        ...
        >
           <div className="w-full h-1 sm:h-1.5 bg-slate-600 absolute top-0 left-0 z-10 rounded-t-lg sm:rounded-t-xl overflow-hidden">
            <div
         ...
         <p className="mt-3 sm:mt-4 text-xs sm:text-sm text-indigo-200">
